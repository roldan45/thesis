...
<body id="main_body">
 <?php
        ...
        where category = 'SKILL' and question_id not in (SELECT question_id FROM exam_child WHERE exam_id = '".$exam_id."')
        ORDER BY RAND() limit 1";
        $result = $conn->query($query);
        if ($_SESSION['question_count'] > 29) 
        {
          include 'observation_questions.php';
        }
        else
        {
          while($row = $result->fetch_assoc())
          {
            echo '
<form method="post" action="skill_questions.php">
      <h5 class="mb-2 mt-4"></h5>
      <div class="col-lg-12 col-12">
          <div class="small-box badge-info">
              <div class="inner"><center>
                  <h3>SKILL</h3>Question '.$_SESSION['question_count'].'</center>
              </div>
          </div>
      </div>
          <div class="card">
            <div class="card-body">
              <div id="questions">
                ...
                <button type="submit" name="btn_submit3" class="btn btn-success" >NEXT <i class="fas fa-arrow-right"></i></button>
             </center>
          </div>
        </div>
      </div>
